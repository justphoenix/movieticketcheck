                    ...
                    c.movies.forEach(m => {
                        ...
                        m.shows.forEach(s => {
                            ...
                            s.plist.forEach(p => {
                                ...
                                // 🌟 核心改进：直接指向猫眼官方 PC 官网的影院/影片详情页（完美支持 Mac 浏览器直达，绝无 403）
                                let maoyanPcUrl = `https://www.maoyan.com/cinemas/${c.cinemaId}`;
                                
                                html += `<a class="show-tag" href="${maoyanPcUrl}" target="_blank" title="点击在 Mac 浏览器中打开猫眼官方影院主页">`;
                                html += `<span class="show-time">🕒 ${p.tm}</span>`;
                                html += `<span class="show-price">￥${price} 官网 ▶</span>`;
                                html += `</a>`;
                            });
